...
2. **Maintain formatting**: Keep emojis and formatting consistent across translations (✅, ❌, ===)
3. **Use natural language**: Translate the meaning, not just word-for-word
4. **Test thoroughly**: Always test the translations in context to ensure they make sense
...
2. At compile time, rust-i18n scans the `locales/` directory and embeds all YAML files into the binary
3. At runtime, the application detects the desired locale from environment variables
4. The `t!()` macro is used throughout the code to translate strings
...
2. Add your locale file following the steps above
